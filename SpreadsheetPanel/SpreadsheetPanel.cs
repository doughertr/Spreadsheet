...
namespace SS
{
    ...
    public partial class SpreadsheetPanel : UserControl
    {
        ...
        /// <summary>
        /// Uses cell value rather than row and col values. Sets the value of
        /// the given cell and returns true.  Otherwise, returns false.
        /// </summary>
        ...
        {
            return drawingPanel.SetValue(cell, value);
        }
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public bool GetValue(string cell, out string value)
        {
            return drawingPanel.GetValue(cell, out value);
        }
        ...
        /// <param name="cell"></param>
        /// <returns></returns>
        public bool SetSelection(string cell)
        {
            return drawingPanel.SetSelection(cell);
        }
        ...
        /// <summary>
        /// Assigns the cell of the current selection to the
        /// out parameters.
        /// </summary>
        /// <param name="cell"></param>
        public void GetSelection(out string cell)
        {
            drawingPanel.GetSelection(out cell);
        }
        ...
        private class DrawingPanel : Panel
        {
            ...
            public bool SetValue(int col, int row, string val)
            {
            ...
            public bool SetValue(string cell, string val)
            {
                int col, row;
                ConvertToRowCol(cell, out col, out row);

                return SetValue(col, row, val);
            }


            public bool GetValue(int col, int row, out string val)
            {
                if (InvalidAddress(col, row))
                {
                    val = null;
                ...
                if (!_values.TryGetValue(new Address(col, row), out val))
                {
                    val = "";
                }
                return true;
            }
            public bool GetValue(string cell, out string val)
            {
                int col, row;
                ConvertToRowCol(cell, out col, out row);

                return GetValue(col, row, out val);
            }
            ...
            public void GetSelection(out string cell)
            {
                ...
                ConvertToString(out cell, col, row);
            }
            ...
            private void ConvertToRowCol(string cell, out int col, out int row)
            {
                string numStringResult;

                char letter;
                int num;

                numStringResult = Regex.Match(cell, @"\d+").Value;

                letter = cell[0];
                num = Int32.Parse(numStringResult);

                col = (int)letter - 65;
                row = num - 1;
            }
            private void ConvertToString(out string cell, int col, int row)
            {
                string letter = ((char)((int)col + 65)).ToString();
                string num = (row + 1).ToString();
                cell = letter + num;
            }
