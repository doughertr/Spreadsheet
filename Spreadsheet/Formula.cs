...
namespace SpreadsheetUtilities
{
    ...
    public class Formula
    {
        ...
        public Formula(String formula, Func<string, string> normalize, Func<string, bool> isValid)
        {
            ...
            {
                string normalizedToken = normalize(token);                          //normalized version of the token if it is a variable
                double num;
                if(double.TryParse(token, out num))                                 //Check if token is a double
                    Tokens.Add(num.ToString());                                     //Add double token into formula list
                else if(token == "+" || token == "-" || token == "*" || token == "/" || token == "(" || token == ")" || token == "^")
                    Tokens.Add(token);                                              //Check to see if token is an operator
                else if (IsVariable(normalizedToken) && isValid(normalizedToken))   //Check if normalized token is a variable and valid
                    Tokens.Add(normalizedToken);                                    //Add token to the underlying list of tokens
                else                                                                //Else token is not valid, therefore throw an error 
        ...
        public object Evaluate(Func<string, double> lookup)
        {
            ...
            double num;
            string errorMessage;
            int currentOperatorRank = -1, previousOperatorRank = -1;
            for (int i = 0; i < Tokens.Count; i++)
            {
                ...
                else if (currentToken == "+" || currentToken == "-" || currentToken == "/" || currentToken == "*")
                {
                    ...
                    if (previousOperatorRank == -1) //-1 rank implies that there is no operator before it
                        previousOperatorRank = currentOperatorRank;
                    else if (currentOperatorRank <= previousOperatorRank)
                    {
